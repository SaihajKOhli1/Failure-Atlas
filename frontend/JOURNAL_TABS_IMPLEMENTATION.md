# Journal Tabs Implementation

## Summary

Implemented tab navigation in `/journal` with query param routing, including:
...
**Behavior**:
- Resets entries when switching repos
...
**Note**: No demo data shown, honest placeholder only.
...
- **Most Recent**: Date of first entry in list (entries sorted by date)
...
1. **`frontend/app/journal/page.tsx`**:
   ...
   - **Line ~671-693**: Updated tabs to be clickable with active state
   - **Line ~578-623**: Updated disconnected state tabs
   - **Line ~625-720**: Added disconnected tab content
   - **Line ~711-938**: Moved entries content to `activeTab === 'entries'` conditional
   - **Line ~941-953**: Added commits tab placeholder
   - **Line ~955-1036**: Added insights tab with real computed stats
   - **Line ~1038-1150**: Added settings tab (User ID + Repo management)
   - **Line ~1121-1142**: Made repos clickable in Settings tab
...
1. **View commits tab**:
   ...
   - ✅ Small muted text explains backend endpoint needed

2. **Check disconnected state**:
   - ✅ No repos connected
   - ✅ Click "Commits" tab
...
1. **View insights with entries**:
   ...
   - ✅ Shows stat cards:
     ...
     - Most Recent: date of first entry

2. **Empty state**:
   - ✅ Connect repo with no entries
   - ✅ All counts show 0, Most Recent shows "—"
...
3. **Disconnected State**:
   ...
   - ✅ Shows User ID section
   - ✅ Shows repo management with "Connect Repo" button
...
Once backend endpoint exists, frontend can:
